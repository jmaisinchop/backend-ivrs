...
export class CampaignController {
  ...
  async createCampaign(
    @Body() createCampaignDto: CreateCampaignDto, // Ahora recibe el DTO completo
    @Req() req
  ) {
    const userId = req.user.id;
    // Se pasa el userId y el DTO completo al servicio
  ...
  @Patch(':id')
  @UsePipes(new ValidationPipe({ whitelist: true })) // whitelist ignora campos que no estén en el DTO
  async updateCampaign(
    @Param('id') campaignId: string,
    @Body() updateDto: UpdateCampaignDto,
  ) {
    // Llama al nuevo método en el servicio que crearemos en el siguiente paso
